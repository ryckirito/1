"""命令行入口。
    ...
    python -m daily_report run --source eastmoney --config config.yaml --date 2026-09-24
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="daily-report", description="A 股每日异动与推荐建仓日报")
    ...
    run.add_argument("-s", "--source", choices=["demo", "csv", "eastmoney", "akshare"], help="数据源，覆盖配置文件")
    ...
    run.add_argument("--capital", type=float, help="账户总资金，用于仓位计算")
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    overrides = {
        "data": {"source": args.source, "csv_dir": args.csv_dir, "max_symbols": args.max_symbols},
