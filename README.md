...
3. **推荐建仓**：多因子评分（权重可配，默认合计 100）筛出 Top N，并给出完整交易计划。
   - 趋势 25：收盘 > MA50 > MA200，MA20 向上（历史不足 200 日时看 MA20 / MA50）
   - 动量 15：20 日涨幅在 0~40% 的区间
...
```bash
python -m daily_report backtest --source yfinance --days 200 --horizon 20 --trades trades.csv
```

在历史上每个交易日按同样规则选股，统计之后 N 日的收益、胜率、相对 SPY 的超额收益、止损/目标触发率，并按形态与评分分组。所有指标只依赖过去数据，每只股票只算一次指标再按日期切片，200 日 × 185 只约 20 秒。

默认参数就是这样定下来的。用 2026-01 到 2026-08 的 185 只大盘股回测，持有 20 日：

| 配置 | 平均收益 | 胜率 | 相对 SPY 超额 | 止损触发 |
|---|---|---|---|---|
| 全池等权（基线） | +1.59% | — | +0.16% | — |
| 初版规则（ATR ≤ 5%、止损 2×ATR / 3%、动量 ≤ 25%、无相对强弱、60 分） | +0.89% | 41% | −0.52% | 53% |
| 当前默认 | +2.05% | 51% | +1.39% | 34% |

初版规则跑输随机选股，主要是 ATR 上限把强势高波动股全部剔除、止损太紧、动量上限太低。只有一段行情的回测，不代表未来；改参数后请再跑一次回测确认。
